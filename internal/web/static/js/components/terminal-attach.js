...
function initTerminalAttach(host) {
  ...
  // The view is brought to the cursor when a person acts: on the first snapshot
  // of a page, on focus, when a pane is come to, and while typing. Never
  // because the cursor moved. A program redrawing its screen walks its cursor
  // all over a pane that, with rows past the fit, is taller than the box, and a
  // cursor move cannot say whether a person or a redraw caused it, so following
  // it pulls the view back and forth under whoever is reading. What a person
  // did is known on this side, and that is what this hangs on.
  const followCursor = () => {
  ...
  // ---- Selection layer -------------------------------------------------------
  // The canvas renderer draws glyphs to a bitmap, so nothing on the screen is
  ...
  const resolveClientSize = async () => {
    ...
    lastClientCols = cols;
    lastFitRows = fit;
    return { cols, rows: Math.max(2, fit) };
  };
  ...
  const keepRows = (size) => {
    ...
    const rows = Math.max(2, lastFitRows);
