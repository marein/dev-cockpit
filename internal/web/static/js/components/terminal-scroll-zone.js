(() => {
  ...
  // terminal-attach.js converts pixels into per-program scroll steps, so content
  ...
  class TerminalScrollZone extends HTMLElement {
    ...
    focusTerminalInput() {
      ...
      const anchor = document.getElementById("terminal-cursor-input");
      if (!anchor) {
        return;
      }
    ...
    blurTerminalInput() {
      ...
      if (active && (active.id === "terminal-prompt" || active.id === "terminal-cursor-input")) {
        active.blur();
      }
    }
    ...
    emitScroll(dy, begin) {
      this.dispatchEvent(new CustomEvent("terminal-scroll", {
        bubbles: true,
        composed: true,
  ...
  customElements.define("terminal-scroll-zone", TerminalScrollZone);
})();
