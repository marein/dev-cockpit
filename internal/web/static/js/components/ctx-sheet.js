import { areaKey, keepCtxScroll } from "@dc/ctx";
...
// own: the projects page filters its cards through a field of its own, the
// column's index and the page's board are two lists on one screen, and one
// writing the other's memory would move a filter nobody typed there.
...
class CtxSheet extends HTMLElement {
  ...
  decorate() {
    ...
    if (input && !column.dataset.ctxFilterWired) {
      ...
      const key = areaKey("filter", this.area);
      input.value = get(key, "");
