		...
		},
		{
			...
			Items: []DocsItem{
				...
				},
				{
					...
					Tag:      "Editor",
					TagClass: "bg-secondary-lt",
					Desc:     `Right-click a tab or a tree row for everything that acts on that one file: copy, download, extract, rename, delete, and how you want to look at it, the preview, the git diff, the diff against a revision, the file history, the git blame and the two compare entries. On touch, long-press; tapping the already active tab also opens its menu. The editor menu <i class="ti ti-dots-vertical align-text-bottom" aria-hidden="true"></i> keeps only what acts on the editor as a whole.`,
				},
				{
					Title: "Reorder tabs",
