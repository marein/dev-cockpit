		...
		},
		{
			...
			Items: []DocsItem{
				...
				},
				{
					...
					Desc:  `For PHP and Go files. Hold <kbd>Ctrl</kbd> (<kbd>Cmd</kbd> on a Mac) and click a symbol to jump to its definition, or to its usages when the cursor already sits on the declaration; <kbd>Ctrl</kbd>+<kbd>B</kbd> does the same for the symbol under the cursor, <kbd>Shift</kbd>+<kbd>F12</kbd> always lists the usages. On touch, a tap on a symbol raises a small <em>Look up</em> action next to it. Typing in the usages list narrows the rows, a click or tap jumps there. A target the project does not hold opens too: a dependency's sources, the Go standard library, a PHP stub. Those open as a read only tab, marked with a lock, and cannot be saved. Looking up works in them like anywhere else, so a jump carries on into the next dependency and back into your own code. The project's language servers start when its editor opens and index in the background: the statusbar shows how far they are, and a lookup waits for the index instead of answering from half of it. <em>Reindex</em> in the editor's menu starts them over. Under Settings, Editor, LSP each language picks how its server runs.`,
				},
				{
					Title: "Git marks",
