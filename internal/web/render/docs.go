			...
			Items: []DocsItem{
				{
					...
					TagClass: "bg-blue-lt",
					Desc:     `On a phone the tab bar's Projects, Terminals, Assistants and Settings do not jump to a page: they slide a sheet up with the area's list, the same rows the desktop shows beside the page. A row opens its page and the sheet goes; the terminals sheet opens with the terminal you are in centered. Projects, terminals and assistants have a filter field, projects the sort menu <i class="ti ti-arrows-sort align-text-bottom" aria-hidden="true"></i> the board uses; the sheet keeps what you typed into its filter until you clear it, and the projects page keeps its own filter over the cards. A split view lists its members as rows of their own under it, each with its actions. A terminal row carries its actions behind <i class="ti ti-dots align-text-bottom" aria-hidden="true"></i>, rename, stop, delete, steer, the split view's own, and the grip <i class="ti ti-grip-vertical align-text-bottom" aria-hidden="true"></i> drags it into a new order, the same order every terminal list shows; a split view travels with its members, and a finger held at the top or bottom scrolls the list.`,
				},
				{
					Title: "The project row's menu",
					Desc:  `The three dots <i class="ti ti-dots align-text-bottom" aria-hidden="true"></i> on a project row, in the list beside the board and in the phone's sheet, open its actions; a right click or a held finger does too: open the project or its editor, start a coder or a shell, git and compose actions, delete.`,
				},
				{
			...
			Items: []DocsItem{
				{
					...
					Desc:  `The sparkle <i class="ti ti-sparkles align-text-bottom" aria-hidden="true"></i> in the rail, in the tab bar and in the terminal switcher opens the assistants where you left off, the one you looked at last, the way the terminals open. It is a page: the assistants stand in the list column beside it, the steered coders in an aside on a wide window and behind the steering wheel in the head below, the memory behind the brain in the list's head. A message carries a stripe on its left, blue for you, purple for the assistant, grey for a report a check wrote.`,
				},
				{
				...
				},
				{
					...
					Tag:      "Memory",
					TagClass: "bg-blue-lt",
					Desc:     `Say remember that and the assistant writes it down. The brain icon <i class="ti ti-brain align-text-bottom" aria-hidden="true"></i> in the head of the list opens what it knows, to read and to correct. The memory is shared by all of them and goes into every answer, so what you tell one they all know.`,
				},
				{
				...
				},
				{
					...
					Tag:      "Voice",
					TagClass: "bg-blue-lt",
					Desc:     `The speaker <i class="ti ti-volume align-text-bottom" aria-hidden="true"></i> on an answer reads it aloud, code blocks left out, German answers in a German voice and English ones in an English voice. Voice mode <i class="ti ti-volume align-text-bottom" aria-hidden="true"></i> in the assistant's head reads every finished answer on its own; the choice stays on this device. Under Settings &rsaquo; Assistants &rsaquo; Voice each engine picks how it runs, Automatic, in Docker, or Off.`,
				},
				{
					Title:    "As many as you want",
					Tag:      "Side by side",
					TagClass: "bg-blue-lt",
					Desc:     `The new-assistant button <i class="ti ti-message-plus align-text-bottom" aria-hidden="true"></i> starts another one, and with more than one coder installed it asks which one answers. They all stay live: each keeps its own thread and the coders it steers, so one can watch the release while another looks at something else. Their row says what each one is called, how many coders it holds and when it last said something, and its menu renames and deletes it. Nothing multiplies by itself and nothing is kept aside, an assistant lives until you delete it, and deleting takes its thread and its files with it.`,
				},
				{
					Title:    "Say the next thing right away",
					Tag:      "Queue",
					TagClass: "bg-blue-lt",
					Desc:     `A message sent while an answer is still on its way waits instead of being refused: it stands in the thread marked Waiting, Don't send takes it back while it waits, and when the answer arrives everything waiting goes out as one message. The queue is that assistant's alone, the others answer at the same time.`,
				},
				{
					Title:    "Put the list in your own order",
					Tag:      "Sorting",
					TagClass: "bg-blue-lt",
					Desc:     `Drag a row where you want it, on a phone by its grip <i class="ti ti-grip-vertical align-text-bottom" aria-hidden="true"></i>. The order is kept on the server, so it comes back after a reload and on every other device, and nothing moves by itself when an assistant answers.`,
				},
				{
					Title: "Step through the assistants",
					Keys:  []DocsKeys{{Caps: []string{"Ctrl", "Tab"}}, {Caps: []string{"Ctrl", "Shift", "Tab"}}},
					Desc:  `Moves through the assistants in the order the list stands in and wraps at either end, the way it steps through the terminals. Browsers often reserve this shortcut; it is reliable in an installed web app.`,
				},
				{
				...
				},
				{
					...
					Tag:      "Tools",
					TagClass: "bg-blue-lt",
					Desc:     `It has the tools of a coder: reading and searching, writing files, running commands, fetching a page from the web, on that coder's default model. It belongs to no project, and the files it makes stay in a workspace of its own next to the cockpit data. A finished answer notifies like any other news and names which assistant answered, and reads itself while that one is open in front of you.`,
				},
				{
				...
				},
				{
					...
					Tag:      "Wakes up",
					TagClass: "bg-blue-lt",
					Desc:     `A job is one coder, one task, and one criterion that decides the task is done. Work an assistant hands over is steered from the start, and you can steer any coder yourself. A steered coder that finishes, asks something or stops moving buys a check, and the check sends the coder what it needs to go on. Only done and blocked reach you. Ten checks and eight hours per job. A coder is steered by exactly one assistant: the reports land in that thread, and nobody else can take it over. Delete an assistant and its coders are yours again, named in the answer.`,
				},
				{
				...
				},
				{
					...
					Tag:      "Ownership",
					TagClass: "bg-blue-lt",
					Desc:     `A steered coder's icon turns purple wherever it shows, and the color follows steer and release without a reload. Steer and release where the coder is listed: the context menu of its tab (on a phone behind the three dots of its row in the sheet), its pane header or its chip. The dialog asks which assistant the reports go to as soon as there is more than one, and its criterion may stay empty, the checks then judge against the task the coder is on.`,
				},
			},
		},
		{
			...
			Items: []DocsItem{
				...
				},
				{
					...
					Desc:  `The gear button <i class="ti ti-settings align-text-bottom" aria-hidden="true"></i> sets the font size and the color theme, stored per device. Every palette follows the OS between a light and a dark variant.`,
				},
				{
			...
			Items: []DocsItem{
				{
					...
					Desc:  `The project name above the file tree, and <em>Switch project</em> in the editor's menu, open a palette over the editor: the last used projects first, then all of them ordered like the projects page with a worktree behind its main, every row naming its repository and branch. Type to narrow by name, repository or branch, arrows and Enter open one, and Enter on a fresh palette goes back to the last used project. A project created or deleted anywhere comes and goes here without a reload. A project comes back exactly as it was left: the open tabs with their cursor and scroll position, a diff included with the unchanged blocks you opened in it, the tree, an open commit or compare view with its filter and list position, and the terminal panel.`,
				},
				{
					Title: "The file tree",
					Desc:  `The folder button <i class="ti ti-folder align-text-bottom" aria-hidden="true"></i> is on every width: on a wide screen it folds the tree column away and back, on a small one it opens the drawer the tree sits in. On a wide screen you can also drag the divider next to the tree to resize it. The fold, the width and the scroll position are remembered per project like the open folders, a project opened for the first time starts with what this device set last, and closing a folder folds everything inside it.`,
				},
				{
					Title: "Quick open",
				...
				},
				{
					...
					Desc:  `Click a line number, or the gutter beside it, to open the line's menu: add or edit the comment there, delete an existing note after a confirm, copy the file's path with the line, and toggle the git blame; a right click and a touch long-press open the same menu, and <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>C</kbd> comments the cursor line directly, opening an existing note for editing. A commented line highlights its gutter in yellow. Comments stick to their line while you edit, renaming or moving a file takes them along, and they are saved with the project, so they survive a reload and follow you to another device. The quoted line is the anchor: when it moved and stands in the file exactly once, the comment follows it on its own, and otherwise it shows as outdated — the line number orange and struck through, the old quote in the list; when its line fell off the file's end, or the file is gone, the list is where it stands. <em>Line comments</em> in the editor menu, or <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>C</kbd>, lists every comment of the project, one cell per comment like the commits in the git sheet: a click on a cell opens its menu with <em>Go to line</em>, <em>Edit</em> and <em>Delete</em>. <em>Copy as Markdown</em> puts them all on the clipboard, file, line and comment with the code line quoted, and <em>Delete all comments</em> clears them.`,
				},
				{
				...
				},
				{
					...
					Desc:  `The commit button <i class="ti ti-git-commit align-text-bottom" aria-hidden="true"></i> above the file tree, or <em>Commit</em> in the git sheet, turns the tree into the list of changes, each with a checkbox, and nothing starts picked. The view stays up for that project until it is closed. Only the checked files go in, exactly as they are in the working copy; whatever a coder has staged for other files stays staged and untouched. Clicking a row shows its diff. Write the message below, <kbd>Ctrl</kbd>+<kbd>Enter</kbd> commits; unsaved picked files are saved first, and <em>Amend</em> rewords or extends the last commit instead; with nothing picked it rewrites only the message. The message, the picks and an amend in progress are saved with the project as you type and tick, so a reload, another browser or another device opens the panel where you left it, and a successful commit clears them everywhere. The list opens grouped by folder, folders first and files after them on every level: a subfolder nests under its parent, a chain of folders with nothing of its own reads as one row, and a folder's checkbox picks and drops everything below it. The folders button <i class="ti ti-folders align-text-bottom" aria-hidden="true"></i> switches to a flat list, each file with its full path under its name, and the choice stays on this device. Conflicted files cannot be picked. <em>Commit and push</em> behind the arrow next to the button pushes right after the commit, to wherever <code>git push</code> goes in this repository; a commit whose push is refused stands, with the refusal in the panel.`,
				},
				{
				...
				},
				{
					...
					Desc:  `<em>Compare revisions</em> in the git sheet lists the files that differ between two revisions. <em>from</em> and <em>to</em> open the revision picker; a typed name such as <code>HEAD~3</code> works too. The select switches between <code>a...b</code> (changes since the split, default) and <code>a..b</code> (all differences). The list behaves like the commit view and stays up for that project the same way. <kbd>Enter</kbd> opens the read only diff of a file between the two revisions.`,
				},
				{
