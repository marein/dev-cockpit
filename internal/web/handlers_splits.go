...
// the project it works in moves to the top of the recent list, the terminal
// itself becomes the one the Terminals entry reopens, and its news counts as
// seen. The attach pages do it while they render, the split page for the pane
// it renders focused, and an open split posts it whenever the focus moves to
// another pane, which on that page happens without a navigation of its own,
// so a pane made active inside a split counts like any other terminal
// somebody looked at. An empty project name touches nothing.
func (s *Server) terminalFocused(id, projectName string) {
	s.projects.Touch(projectName)
	s.terminalRecent.Touch(id)
