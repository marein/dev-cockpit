...
func TestTheOldAssistantPathsLeadToTheNewOnes(t *testing.T) {
	...
	moved := []struct{ method, from, to string }{
		...
		// What the released CLI called over the socket forwards like the rest:
		// no released CLI reaches this server, see AGENTS.md, and the local
		// API client of this build calls the plural path itself.
		{http.MethodGet, "/assistant/jobs", "/assistants/jobs"},
		{http.MethodGet, "/assistant/conversations", "/assistants/conversations"},
...
func TestTheAssistantAreaIsServedInThePlural(t *testing.T) {
	...
	want := map[string]bool{
		...
		"GET /assistants/new":                     false,
		"POST /assistants/new":                    false,
