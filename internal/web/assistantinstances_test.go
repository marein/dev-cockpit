...
func TestInstanceRoutesServeTheAssistantReads(t *testing.T) {
	...
	store.Save(assistant.Instance{
		Summary: assistant.Summary{ID: tabs, Title: "Fix the tabs", CoderID: "claude", Status: assistant.StatusActive},
	...
	store.Save(assistant.Instance{
		Summary:  assistant.Summary{ID: plans, Title: "Weekend plans", CoderID: "claude", Status: assistant.StatusActive},
...
func TestMediaAddressServesItsOwnAssistantsWorkspace(t *testing.T) {
	...
	for _, id := range []string{mine, theirs} {
		store.Save(assistant.Instance{Summary: assistant.Summary{ID: id, Title: id, CoderID: "claude", Status: assistant.StatusActive}})
	}
	conversations, workspace, err := assistant.New(stateDir, oneCoder{}, assistant.Cockpit{StateDir: stateDir})
	if err != nil {
		t.Fatalf("assistant: %v", err)
