...
// PendingReviewCount returns how many overwrite reviews are open, a cheap
// load-and-count (no per file stat) for the nav badge rendered on every
// page. The backup page itself prunes stale entries.
func (s *Service) PendingReviewCount() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return len(s.loadReview())
}
