...
// reading of the same log still names the tools, it answers the question what
// the session last did.
func TestTheCopyReadingLeavesTheToolCallsOut(t *testing.T) {
	lines := []string{sessionStartEvent, userEvent, turnStartEvent, toolEvent, toolDoneEvent, answerEvent, turnEndEvent}
	recorded, err := readTranscript(strings.NewReader(strings.Join(lines, "\n")), 0, 0)
	if err != nil {
