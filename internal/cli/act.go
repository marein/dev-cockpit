...
func newStopCoderCommand(opts *inspectOptions) *cobra.Command {
	return &cobra.Command{
		...
		Long: "Stop a coder session the way the stop button does: the terminal goes away, the " +
			...
			"identifier. A job steering this coder ends with it, because a stopped " +
			"coder cannot report again, while a trigger on this terminal stands: the " +
			"session keeps its identifier.",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newCoderCommand(opts *inspectOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Start a coder session the same way the new coder form does. The project is " +
			...
			"hangs on a steered job, and it expires with that job: when the job is over, so " +
			"is the sequel.",
...
func runNewCoder(out io.Writer, opts inspectOptions, project, name, coderID, agent, model, prompt, doneWhen, then string) error {
	...
	// The sequel travels with the same request as the criterion it hangs on: a
	// job can close before a second call could make the trigger, and that race
	// is exactly what a handover must not have.
	...
	fmt.Fprintf(out, "trigger %v fires once when that job is done\n", created["trigger"])
	return nil
}
