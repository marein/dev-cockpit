...
type TurnRequest struct {
	...
	Resume bool
	// Title names the provider session on creation, so a transferred instance
	// shows up as a coder terminal with the instance's title.
	Title string
...
// Terminals promotes an assistant's provider session into a coder terminal. The web
...
type Terminals interface {
	...
	ResumeReserved(coderID, sessionID, workdir, title string) (string, error)
