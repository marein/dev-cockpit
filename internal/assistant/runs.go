...
const (
	...
	// RunReaction is a turn an event asked for, through a trigger.
...
type RunRecord struct {
	...
	// Origin is what a reaction needs to be concluded by whoever finds it: the
	// event and the task, the trigger that fired, and MessageID is the id its
	// answer is pushed under, so a reaction concluded after a restart pushes
	// exactly one message.
