...
type RunRecord struct {
	...
	// Owner is the assistant this turn acts as, which is what the turn's
	// process is told about itself. A chat turn's owner is the assistant it
	// writes into; a check's is the assistant that steers the job, which is why
	// this is its own field and not read off Instance: a check has no
	// transcript entry, and it still has to act as somebody.
