...
func (s *Workspace) instructions() string {
	...
	if strings.TrimSpace(s.cockpit.Executable) != "" {
		...
		b.WriteString("```bash\n")
		fmt.Fprintf(&b, "%s <project> [--path <expr>]... [--contains <word>]   # the notes, each with its id\n", s.CockpitCommand("line-comment-list"))
		fmt.Fprintf(&b, "%s <project> <path> <line> <text>   # pin a note to a 1-based line of a project relative file\n", s.CockpitCommand("line-comment-add"))
		fmt.Fprintf(&b, "%s <project> <id>... | --path <expr>...   # remove notes, --path clears the matching files\n", s.CockpitCommand("line-comment-remove"))
		b.WriteString("```\n\n")
		b.WriteString("The quoted code line is read from the file when you add a note, so you never pass it yourself. ")
		b.WriteString("The quote is also the anchor: every read holds the quotes against the files, a quote that stands ")
		b.WriteString("in its file exactly once follows it to the new line on its own, and one that is gone, ambiguous, ")
		b.WriteString("or whose file is missing reads as outdated, still at its last known line with the old quote. ")
		b.WriteString("Outdated is judged fresh on every read and never stored, so a file that comes back heals its ")
		b.WriteString("comments by itself. A file renamed or moved in the editor takes its notes to the new path, ")
		b.WriteString("so the paths in the list stay current. `--outdated` narrows `line-comment-list` and `line-comment-remove` to exactly ")
		b.WriteString("those, combines with `--path`, and `line-comment-remove --outdated` is how the orphans are ")
		b.WriteString("cleaned up once they are truly stale.\n\n")
		b.WriteString("`--path` works the same on list and remove: each value is an exact file, a folder that takes ")
		b.WriteString("everything under it, or a glob where `*` stays inside a path segment and `**` crosses them, ")
		b.WriteString("like `--path '**/assistant.go'`, and the flag may repeat, the values or together. ")
		b.WriteString("`line-comment-list` is capped at the first ones and says how many more there are; `--path` and ")
		b.WriteString("`--contains` narrow it before the cap, so they reach every note the plain list never shows. ")
		b.WriteString("Every change reaches the open editors live. The line comments are how the user reviews code: ")
		b.WriteString("a review reaches you through `line-comment-list`, or when the user pastes it into a message, ")
		b.WriteString("and either way those comments are the work. ")
		b.WriteString("Once you hand comments to a coder for fixing, remove exactly the ")
		b.WriteString("ones you handed over with `line-comment-remove`, right after the handoff, without waiting for a ")
		b.WriteString("confirmation or for the job's DONE: the handoff is what spends a note, the coder carries it from ")
		b.WriteString("there, and the list stays what nobody is on yet.\n\n")
	}
