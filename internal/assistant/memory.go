...
// shortCockpit is how every example spells a cockpit command: the wrapper by
// its name in the directory the turn starts in. The absolute path stands once
// beside it, for a turn that changed into a project.
const shortCockpit = "./" + wrapperFileName
...
type instructionsData struct {
	...
	FilesDir         string
	MemoryDir        string
	// Cockpit is how a command is spelled in every example, the wrapper by
	// its short name in the directory the turn starts in; Wrapper is the same
	// script by its absolute path, named once for the moments a turn stands
...
func (s *Workspace) instructions(instanceID string) string {
	return render("instructions.md.tmpl", instructionsData{
		...
		Cockpit:          shortCockpit,
		Wrapper:          s.Wrapper(instanceID),
...
// Wrapper is the absolute path of one assistant's wrapper in its workspace,
