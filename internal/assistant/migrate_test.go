...
func seedLegacy(t *testing.T, root string, index []Summary, transcripts map[string]Instance, jobs []Job) {
	t.Helper()
...
func TestMigrationKeepsTheLiveConversationAndDropsTheRest(t *testing.T) {
	...
	seedLegacy(t, root,
		[]Summary{
			{ID: live, Title: "Live one", CoderID: "claude", Status: StatusActive, LastMessageAt: now},
			{ID: old, Title: "Old one", CoderID: "claude", Status: "archived", LastMessageAt: now.Add(-time.Hour)},
		},
		map[string]Instance{
			live: {Summary: Summary{ID: live, Title: "Live one", CoderID: "claude", Status: StatusActive}},
			old:  {Summary: Summary{ID: old, Title: "Old one", CoderID: "claude", Status: "archived"}},
		},
	...
	if text := mustRead(t, filepath.Join(own, "CLAUDE.md")); text == "generated" || !strings.Contains(text, "the id `"+live+"`") {
...
func TestMigrationDropsATranscriptTheIndexNeverKnew(t *testing.T) {
	...
	live := "11111111-1111-4111-8111-111111111111"
	seedLegacy(t, root,
		[]Summary{{ID: live, Title: "Live one", CoderID: "claude", Status: StatusActive}},
		map[string]Instance{live: {Summary: Summary{ID: live, Title: "Live one", CoderID: "claude", Status: StatusActive}}},
		nil,
	)
...
func TestMigrationWithoutALiveConversationKeepsNothing(t *testing.T) {
	...
	old := "22222222-2222-4222-8222-222222222222"
	seedLegacy(t, root,
		[]Summary{{ID: old, Title: "Old one", CoderID: "claude", Status: "archived"}},
		map[string]Instance{old: {Summary: Summary{ID: old, Title: "Old one", CoderID: "claude", Status: "archived"}}},
