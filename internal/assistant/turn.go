...
func reactionOutcome(text string, turnErr error, cancelled bool) (wakeOutcome, error) {
	switch {
	case cancelled:
		return wakeOutcome{}, errors.New("The reaction was stopped.")
	case turnErr != nil:
		return wakeOutcome{}, turnErr
	case strings.TrimSpace(text) == "":
		return wakeOutcome{}, errors.New("The reaction ended without saying anything.")
	}
	return wakeOutcome{Text: strings.TrimSpace(text)}, nil
}
...
func (s *Service) orphan(rec RunRecord, cause error) {
	...
	case RunReaction:
		// Nobody waits on a lost reaction, so its subscription is told here.
		a.err = cause
		s.runs.Delete(rec.ID)
		s.events.conclude(rec, wakeOutcome{}, cause)
	default:
