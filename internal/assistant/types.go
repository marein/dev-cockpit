...
// Status is the lifecycle state of an assistant.
type Status string

const (
	// StatusActive marks an assistant that still owns its provider session.
	StatusActive Status = "active"
	// StatusTransferred marks an assistant whose provider session was handed to
	// a coder terminal. The transcript stays readable, the composer does not.
...
type Instance struct {
	Summary
	// NativeSessionID is the provider session this instance drives. It equals the
	// instance id, which is UUID shaped so it also works as a tmux session name
	// once the instance is transferred.
