...
// so a state directory that has already moved is not looked at twice. The
// last release made that directory with the first transcript it saved and
// not before, while its memory sheet wrote into the shared workspace from the
// first page on, so a state directory that only ever held memory has the
// workspace and no transcript directory: that shape is the old layout too,
// as long as nothing of the new one exists beside it, and the move takes it
// the same way, the memory up and the rest gone. This is the only move there
// is: it reads the layout of the last release and nothing else, and a state
// directory in any other shape is left as it is.
const (
...
// legacySummary is an index entry as the last release wrote it: with a status,
// active for the one conversation that was live and archived for every other.
// An assistant has no status any more, it lives until it is deleted, so the
// word is read here and nowhere else, and the entry this move keeps is written
// back without it. An index that still carries the key loads as before, an
...
// legacyStatusActive marked the conversation that was live.
const legacyStatusActive = "active"

// migrate turns the old one-conversation layout into instances. It is a no-op
// once nothing of the old layout is left, which is the state every fresh
// install starts in.
func migrate(root, indexPath, instancesDir, memoryDir string) error {
	if !legacyLayout(root, instancesDir, memoryDir) {
		return nil
	}
	legacy := filepath.Join(root, legacyTranscripts)

	var index []legacySummary
	...
	for _, entry := range index {
		if entry.Status != legacyStatusActive {
			continue
		}
		...
		kept = entry.ID
		carried = append(carried, entry.Summary)
		break
	}
...
// legacyLayout reports whether a state directory still holds the shape of the
// last release: its transcript directory, or its shared workspace with
// nothing of the new layout next to it. The second reading is guarded by the
// new layout on purpose: a workspace directory lingering beside instances
// that already exist is a leftover, and moving it again would drop the memory
// that already moved and write an empty index over the assistants that live.
func legacyLayout(root, instancesDir, memoryDir string) bool {
	if isDir(filepath.Join(root, legacyTranscripts)) {
		return true
	}
	return isDir(filepath.Join(root, legacyWorkspace)) && !isDir(instancesDir) && !isDir(memoryDir)
}
