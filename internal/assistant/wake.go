...
// startReaction spends one turn on an event a trigger fired, the way a check
// is spent: a provider session of its own, the wake slot, the register. The
...
func (s *Service) recordWake(job Job, messageID string, verdict Verdict, text string) string {
	...
	// The report is also an event: a trigger on this job's end fires on it, in
