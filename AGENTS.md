...
- **Several assistants live side by side, and a job belongs to one of them.**
  ...
  and the surfaces call it when a person asks. `Service.Newest` is what a
  surface opens when it has to open something without being told which.
  ...
  **A job carries its owner**, and the owner is the directory it was read from
  ...
  Deleting an assistant releases its jobs first and the answer names the coders
  that came back. The jobs live on one path, `/assistants/jobs`, which serves
...
- **One chat turn per assistant, and the queue is that assistant's.** A second
  ...
  wait, `Transfer` refuses while something does. Across assistants there is no
...
- **An assistant knows who it is from its own instruction file.** The
  ...
  workspace.
- **A cockpit command is one word, the wrapper.** Every workspace carries a
  ...
  `cockpitNamer` the workspace implements) name it by its absolute path in
  every example, because a turn changes into other directories, and they name
  nothing else, so a check reads one spelling of a command and a turn cannot
  drop or misspell a flag. A caller without a workspace to ask (the plain
  `wakePrompt`) falls back to `dev-cockpit assistant`.
- **The texts the cockpit writes for an assistant are templates.**
  ...
  wrapper, the check prompt (`wake_prompt.md.tmpl`, it also sends a check to
  the project's own instruction files when a criterion touches conventions or
  gates, naming no file list because the names differ per project and coder)
  and the three reports a check ends in. They read like text and are edited as
  ...
  a template, so a pinned sentence in a test matches the file.
...
- **The area is `/assistants`, and the old subtree keeps landing.** Every
  ...
  for the phone's sheet. Three addresses in the old subtree answer in place
  instead: `/assistant/jobs` and the two `/assistant/conversations` reads, what
  a released CLI calls over the socket. The local API client never follows a
  redirect, it wants the JSON, so a 308 there would not move it, it would break
  it. The area is plural everywhere it is named, like Projects and Terminals,
...
- **The assistants are a page.** `/assistants` is the area's entry and decides
  ...
  `/ctx/assistants?path=` on the assistant event, its rows carry the assistant
  ...
  The steered coder rows' actions are `btn-sm`. A transferred assistant renders
  read-only on its own address; deleting one sends the browser to
  `/assistants`; a notification names `/assistants/<id>#message-<id>`
