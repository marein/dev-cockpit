...
L.runFeature("wake", async ({ page, run }) => {
  ...
  await run("a coder that reports gets checked, and a finished job reaches the user", async () => {
    ...
    // The title says how the job ended before it says anything else, because
    // the front of the line is the part every surface shows. The job's own
    // name rides behind it, from the report's note and without any lookup,
    // while there is room for it.
    assert(news.length >= 1 && /^Job done[ ,.]/.test(news[0].title || ""),
      `unexpected news ${JSON.stringify(news)}`);
    // The line below it is the report itself: an entry that only said "Job
    // done." left the user without a word about what the coder did.
    assert(/job is finished/.test(news[0].detail || ""),
      `the news does not carry the report: ${JSON.stringify(news[0])}`);
