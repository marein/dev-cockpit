...
// Custom elements session-attach, session-input, session-scroll-zone,
// session-direction-pad, session-file-upload, session-terminal-setting-select. The
...
// GET/POST /sessions/new, GET /sessions/:id, POST /sessions/:id/{stop,input,resize},
// GET .../stream, .../files (+POST upload, /download, /delete), POST /resumable/:id/{resume,delete}.
...
L.runFeature("SESSIONS", async ({ page, run, mobilePage }) => {
  ...
  try {
    ...
    await run("create -> attach elements + canvas", async () => {
      ...
      assert((await L.waitUpgraded(page, ["session-attach", "session-input", "session-file-upload", "session-terminal-setting-select"], 12000)).length === 0, "not upgraded");
    ...
    await run("desktop: prompt modal sends to /input, closes, agent reacts", async () => {
      ...
      await modalShown(page, "session-prompt-modal"); await sleep(600);
      ...
      await page.waitForFunction(() => { const m = document.getElementById("session-prompt-modal"); return m && !m.classList.contains("show"); }, null, { timeout: 6000 });
    ...
    await run("files: multi upload -> Done -> list, reference (Copied), download, delete", async () => {
      ...
      await page.waitForSelector("#session-terminal .xterm-screen canvas", { timeout: 12000 });
      await page.click(".attach-desktop .session-files-button");
      await modalShown(page, "session-files-modal");
      const f1 = `u1_${tag.slice(-4)}.txt`, f2 = `u2_${tag.slice(-4)}.txt`, content = `payload ${tag}\n`;
      await page.setInputFiles('#session-files-modal input[type="file"][name="files"]', [{ name: f1, mimeType: "text/plain", buffer: Buffer.from(content) }, { name: f2, mimeType: "text/plain", buffer: Buffer.from("two\n") }]);
      await page.click('#session-files-modal [data-session-file-upload-form] button[type="submit"]');
      await page.waitForFunction(() => document.querySelectorAll('#session-files-modal [data-file-index]').length >= 2, null, { timeout: 8000 });
      await page.waitForFunction(() => { const s = [...document.querySelectorAll('#session-files-modal [data-file-status]')].map((e) => e.textContent.trim()); return s.length >= 2 && s.every((x) => x === "Done"); }, null, { timeout: 15000 });
      await page.waitForFunction((ns) => { const c = document.querySelector("[data-session-files-content]"); return c && ns.every((n) => c.textContent.includes(n)); }, [f1, f2], { timeout: 10000 });
      // reference + download the first file
      const copyBtn = page.locator("#session-files-modal [data-copy-file-path]").first();
      await copyBtn.click();
      await page.waitForFunction(() => /Copied/.test((document.querySelector("[data-copy-file-path]") || {}).innerHTML || ""), null, { timeout: 3000 });
      const href = await page.locator('#session-files-modal a[href*="/files/download"]').first().getAttribute("href");
      ...
      const beforeLinks = await page.locator('#session-files-modal a[href*="/files/download"]').count();
      await page.click("#session-files-modal form[data-session-file-delete] button[type=\"submit\"]");
      ...
      await page.waitForFunction((n) => document.querySelectorAll('#session-files-modal a[href*="/files/download"]').length < n, beforeLinks, { timeout: 10000 });
    ...
    await run("mobile: prompt modal opens from the mobile toolbar", async () => {
      ...
      await mp.waitForSelector("#session-terminal .xterm-screen canvas", { timeout: 12000 }); await sleep(800);
      await mp.locator(".attach-mobile [data-session-prompt-modal-open]").first().click();
      await modalShown(mp, "session-prompt-modal");
      assert(await mp.$("#session-prompt-modal-text"), "prompt textarea missing on mobile");
    ...
    await run("resumable: stop -> resumable entry -> resume -> delete", async () => {
      ...
      await page.waitForSelector('form[action^="/resumable/"][action$="/resume"]', { timeout: 8000 });
      await Promise.all([page.waitForURL(/\/sessions\/[^/]+$/, { timeout: 20000 }), page.locator('form[action$="/resume"]').first().locator('button[type="submit"]').first().click()]);
      sessionUrl = page.url();
      await page.waitForSelector("#session-terminal .xterm-screen canvas", { timeout: 15000 });
      await L.stopSession(page, sessionUrl); sessionUrl = null;
      await page.goto(`${BASE}/projects`, { waitUntil: "domcontentloaded" });
      const before = await page.locator('form[action^="/resumable/"][action$="/delete"]').count();
      assert(before >= 1, "no resumable to delete");
      await page.locator('form[action^="/resumable/"][action$="/delete"]').first().locator('button[type="submit"]').first().click();
      await confirmSwal(page); await sleep(800);
      assert(await page.locator('form[action^="/resumable/"][action$="/delete"]').count() < before, "resumable row not removed");
    });
  } finally {
    if (sessionUrl) await L.stopSession(page, sessionUrl).catch(() => {});
    await page.goto(`${BASE}/projects`, { waitUntil: "domcontentloaded" }).catch(() => {});
    for (let i = 0; i < 4; i++) { const d = page.locator('form[action^="/resumable/"][action$="/delete"]').first(); if (await d.count() === 0) break; await d.locator("button").first().click().catch(() => {}); await confirmSwal(page).catch(() => {}); await sleep(500); await page.goto(`${BASE}/projects`, { waitUntil: "domcontentloaded" }).catch(() => {}); }
