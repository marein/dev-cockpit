...
L.runFeature("SPLIT VIEW", async ({ browser, page, run, mobilePage, engine }) => {
  ...
  try {
    ...
    await run("a pane clips: no scrollbar in a 2x2 grid or a stacked column, not even under an oversize", async () => {
      const own = [];
      ...
      const checkState = async (members, label) => {
        ...
        for (const b of plain) {
          assert(b.overflow === "clip", `${label}: the host is not clipped: ${JSON.stringify(b)}`);
        ...
        return { plain: plain.map((b) => `${b.screenW}x${b.screenH} in ${b.cw}x${b.ch}`).join(", ") };
      };
      try {
        ...
        return `2x2 ${gridResult.plain}; stacked ${stackResult.plain}`;
      } finally {
        for (const id of own) {
