...
L.runFeature("PROJECTS", async ({ engine, page, run, mobilePage }) => {
  ...
  try {
    ...
    // A phone carries two filters over the projects at once: the board's own
    // field in the work head, and the list column's, which the sheet brings up.
    // They are two lists, so each remembers its own query, the board under
    // dc-project-filter and the sheet under dc-ctx-filter:projects, the key
    // shape the column's width and scroll already use. Neither writes the
    // other's memory: a query typed in the sheet that also moved the board's
    // field would hide cards nobody asked to hide, and the stored board filter
    // would stand in the sheet the next time it opens.
    await run("at 390px the sheet's filter and the board's filter keep their own memory", async () => {
      const mp = await mobilePage();
      const sheetField = "dc-ctx-sheet [data-ctx-filter]";
      const stored = (key) => mp.evaluate((k) => localStorage.getItem(k), key);
      const value = (sel) => mp.$eval(sel, (el) => el.value);
      const sheetRow = (name) => mp.$eval(`dc-ctx-sheet [data-index-project="${name}"]`, (el) => el.classList.contains("d-none"));
      const cardShown = (name) => mp.evaluate((n) => { const el = document.getElementById(`project-${n}`); return !!el && el.offsetParent !== null; }, name);
      const openSheet = async () => {
        await mp.click('.dc-tabbar button[data-ctx-area="projects"]');
        await mp.waitForSelector(`dc-ctx-sheet:not([hidden]) [data-index-project="${project}"]`, { state: "attached", timeout: 8000 });
        await sleep(300);
      };
      ...
      await L.dismissUpdate(mp);
      try {
        await mp.fill("[data-project-filter]", project);
        await sleep(300);
        assert(await stored("dc-project-filter") === project, `the board's filter is stored as ${JSON.stringify(await stored("dc-project-filter"))}`);

        await openSheet();
        assert(await value(sheetField) === "", `the board's filter stands in the sheet as '${await value(sheetField)}'`);
        assert(!(await sheetRow(source)), "the sheet hides a row the board's filter hides");

        await mp.fill(sheetField, source);
        await sleep(300);
        assert(await sheetRow(project), "the sheet's own filter does not hide a row");
        assert(await stored("dc-ctx-filter:projects") === source, `the sheet's filter is stored as ${JSON.stringify(await stored("dc-ctx-filter:projects"))}`);
        assert(await stored("dc-project-filter") === project, `the sheet wrote the board's memory: ${JSON.stringify(await stored("dc-project-filter"))}`);

        await closeSheet();
        assert(await value("[data-project-filter]") === project, `the sheet moved the board's field to '${await value("[data-project-filter]")}'`);
        assert(await cardShown(project) && !(await cardShown(source)), "the board no longer stands on its own filter");

        await mp.reload({ waitUntil: "domcontentloaded" });
        await L.dismissUpdate(mp);
        await mp.waitForSelector("[data-project-filter]", { state: "visible", timeout: 8000 });
        assert(await value("[data-project-filter]") === project, `the board forgot its filter: '${await value("[data-project-filter]")}'`);
        await openSheet();
        assert(await value(sheetField) === source, `the sheet forgot its filter: '${await value(sheetField)}'`);
        assert(await sheetRow(project), "the remembered sheet filter hides nothing on open");
      } finally {
        await mp.evaluate(() => { localStorage.removeItem("dc-ctx-filter:projects"); localStorage.removeItem("dc-project-filter"); });
        await mp.goto(`${BASE}/projects`, { waitUntil: "domcontentloaded" });
      }
    });
