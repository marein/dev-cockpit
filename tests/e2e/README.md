...
covered inside the feature that owns it, mainly `terminal.js` and `sessions.js`.
...
name. Most scripts run against a copilot instance on `:3010` (cookie `tc_session`).
Extra instances:

- claude on `:3011` (cookie `tcc_session`) for `provider-claude.js`
  (`BASE_URL=https://localhost:3011`).
- copilot on `:3012` started with `DEV_COCKPIT_UPDATE_API_URL` pointing at a stub
...
- copilot on `:3013` with the stub URL returning `[]`, for `update.js MODE=uptodate`.
