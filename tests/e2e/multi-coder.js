...
L.runFeature(`MULTI-CODER (${MODE})`, async ({ page, run }) => {
  ...
  await run("agents: sidebar coder rows + canonical URLs scope list and create", async () => {
    ...
    assert(JSON.stringify(sections) === JSON.stringify(["/settings/coders/copilot/instructions", "/settings/coders/copilot/agents", "/settings/coders/copilot/skills", "/settings/coders/copilot/models"]), `section tabs wrong: ${sections}`);
