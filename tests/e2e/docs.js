...
L.runFeature("DOCS", async ({ page, run }) => {
  ...
  await run("named controls carry their own icon", async () => {
    await page.goto(`${BASE}/docs`, { waitUntil: "domcontentloaded" });
    // One control per topic, each one named in the text beside its icon: the
    // coder's files button, the stream refresh, the editor's preview, and the
    // git button on a project row.
    for (const [topic, icon] of [["terminals", "ti-upload"], ["terminals", "ti-refresh"], ["editor", "ti-eye"], ["navigation", "ti-brand-git"]]) {
