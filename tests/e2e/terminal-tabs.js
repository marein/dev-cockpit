...
// per area, under dc-ctx-filter:<area> like the column's width and scroll: a
// stored filter already hides the rows it hides when the sheet opens, and
// clearing it is stored too. The key is the sheet's own, the projects page
// keeps its board's filter under dc-project-filter and neither moves the
// other. The strip
...
L.runFeature("TERMINAL-TABS", async ({ browser, page, run, mobilePage }) => {
  ...
  try {
    ...
    await run("the settings menu adjusts the font size and stays open while selecting", async () => {
      ...
      assert(selects === 2, `settings menu carries ${selects} selects, expected font size and theme`);
      const fontSel = '.attach-settings terminal-setting-select[setting="font-size"] select';
      const before = await page.$eval(fontSel, (el) => el.value);
      await page.selectOption(fontSel, "16");
      await sleep(300);
      assert(await page.evaluate(() => localStorage.getItem("dc-terminal-font-size")) === "16", "font size not persisted from the settings menu");
      assert((await page.locator(".attach-settings .dropdown-menu.show").count()) === 1, "settings menu closed on select");
      await page.selectOption(fontSel, before);
      await sleep(200);
      await page.keyboard.press("Escape");
      await sleep(200);
    });
    ...
    await run("closing the last tab lands in the terminals area, never on the POST url", async () => {
      ...
      await L.confirmSwal(page);
      // Without a neighbour the close hands over to /terminals, and the entry
      // resolves from there: a terminal this runner still has running on the
      // shared instance, or the area's empty page when there is none. Both are
      // the terminals area, which is the point; what must never show is the
      // POST url or an error page.
      await page.waitForURL(/\/(terminals|coders|shells)(\/|$|\?)/, { timeout: 15000 });
      assert(!/\/stop$/.test(page.url()), `landed on the POST url: ${page.url()}`);
      await page.waitForSelector("terminal-tabs", { state: "attached", timeout: 8000 });
      const body = await page.textContent("body");
      assert(!/Method not allowed/i.test(body), "the landing is an error page");
    ...
    await run("a terminal fits its box, and the page does not grow with its output", async () => {
      ...
      const box = await page.evaluate(() => {
        ...
        return {
          spare: host.scrollHeight - host.clientHeight,
          pageSpare: body.scrollHeight - body.clientHeight,
          overflow: getComputedStyle(host).overflow,
        };
      });
      assert(box.spare <= 1, `the terminal is taller than its box: ${JSON.stringify(box)}`);
      assert(box.pageSpare <= 1, `the page grew with the terminal: ${JSON.stringify(box)}`);
      assert(box.overflow === "clip", `the terminal is not clipped: ${JSON.stringify(box)}`);
      // The cursor is in view because the terminal is the screen, and typing
      // keeps it there; there is no second place it could be.
      const cursorShown = await page.evaluate(() => {
        ...
        return cursor.top >= host.top - 1 && cursor.bottom <= host.bottom + 1;
      });
      assert(cursorShown, "the cursor is not inside the terminal's box");
    });
    ...
    await run("the strip stays hidden on coarse pointer (mobile) clients", async () => {
      ...
      const fontSel = '.attach-settings terminal-setting-select[setting="font-size"] select';
      const before = await mp.$eval(fontSel, (el) => el.value);
      await mp.selectOption(fontSel, "16");
      await sleep(200);
      assert(await mp.evaluate(() => localStorage.getItem("dc-terminal-font-size")) === "16", "font size not persisted from the mobile settings menu");
      await mp.selectOption(fontSel, before);
      await sleep(200);
    });
    ...
    await run("the sheet remembers the terminals filter across a reload", async () => {
      ...
      const stored = () => mp.evaluate(() => localStorage.getItem("dc-ctx-filter:terminals"));
